...
// Controles de filtrado por tipo de transacción
import Filter from "components/MDFilter";
...
function TransactionHistory({ numRows }) {
  ...
  const filteredAndSearchedTransactions = useMemo(() => {
    ...
    const hasActiveFilter = typeFilter.activation || typeFilter.recharge || typeFilter.purchase;

    if (hasActiveFilter) {
      result = result.filter((transaction) => {
        const type = transaction.type;
        if (typeFilter.activation && type === "activation") return true;
        if (typeFilter.recharge && type === "recharge") return true;
        if (typeFilter.purchase && type === "order") return true;
        return false;
      });
    }
    ...
    return result;
  }, [transactions, searchTerm, typeFilter, numRows]);

  // --- Manejo de Estados de Carga/Error ---

  if (loading && !refreshing) {
    return <StateMessage state="loading" message="Cargando transacciones…" />;
  }
  if (error || !data?.event_id || !transactions) {
    return <StateMessage state="error" message="Error al obtener los datos. Intente recargar." />;
  }

  // Si no hay transacciones después de cargar
  if (transactions.length === 0) {
    return <StateMessage state="empty" message="No hay transacciones registradas para este evento." />;
  }
  ...
  return (
    <MDBox pt={3} pr={2} pl={2} pb={3}>
      ...
      <MDBox my={1}>
        {/* Asegúrate de que el componente Filtro envía los cambios de estado a setTypeFilter */}
        <Filter onFilterChange={setTypeFilter} />
      </MDBox>
