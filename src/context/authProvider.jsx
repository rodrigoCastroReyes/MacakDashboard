import React, { createContext, useContext, useState, useEffect } from 'react';
import axios from 'axios';
import { useNavigate } from 'react-router-dom';

// URL
import { API_BASE_URL } from '../config';
import { clearApiCache } from 'utils/apiCache';

const AuthContext = createContext();

export const AuthProvider = ({ children }) => {
  const [authToken, setAuthToken] = useState(
    localStorage.getItem("authToken") || null
  );
  const [managerAdminUserId, setManagerAdminUserId] = useState(null);
  const [userId, setUserId] = useState(null);
  const navigate = useNavigate();

  useEffect(() => {
    const checkAuthentication = async () => {
      if (!authToken) return;
    };
    checkAuthentication();
  }, [authToken, managerAdminUserId, userId]);

  const login = async (username, password) => {
    try {
      const baseUrl = API_BASE_URL.endsWith('/') ? API_BASE_URL.slice(0, -1) : API_BASE_URL;
      const response = await axios.post(`${baseUrl}/user/login`, { username, password });
      const { id, jwtoken, event_id, role } = response.data;
  
      if (role !== "manager_admin") {
        throw new Error('Error al iniciar sesión. Por favor, verifica tus credenciales.');
      }

      localStorage.setItem('authToken', jwtoken);
      localStorage.setItem('eventId', event_id);
  
      // Obtener nombre del evento
      const eventResponse = await axios.get(`${API_BASE_URL}/event?id=${event_id}`, {
        headers: {
          'Authorization': jwtoken
        }
      });
  
      localStorage.setItem("eventName", eventResponse.data.name || "Evento sin nombre");
  
      // Setear estados del contexto
      setAuthToken(jwtoken);
      setUserId(id);
  
      // Obtener info del manager_admin
      const managerAdminResponse = await axios.post(`${API_BASE_URL}/manager_admin`, {
        user_id: id,
        event_id: event_id
      });
  
      const { user_id } = managerAdminResponse.data;
      setManagerAdminUserId(user_id);
  
      return role;
    } catch (error) {
      console.error('Error al iniciar sesión:', error);
      throw new Error('Error al iniciar sesión. Por favor, verifica tus credenciales.');
    }
  };  

  const logout = () => {
    clearApiCache(); // si no, el siguiente usuario vería datos del evento anterior
    localStorage.removeItem('authToken');
    localStorage.removeItem('eventId');
    localStorage.removeItem("eventName");
    setAuthToken(null);
    setManagerAdminUserId(null);
    setUserId(null);
    navigate('/authentication/sign-in');
  };

  return (
    <AuthContext.Provider value={{ authToken, isAuthenticated: Boolean(authToken), login, logout }}>
      {children}
    </AuthContext.Provider>
  );
};

export const useAuth = () => {
  return useContext(AuthContext);
};